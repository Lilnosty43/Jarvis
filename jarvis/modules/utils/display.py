...
class Echo:
    ...
    def critical(self, msg: Any, add_prefix: bool = True) -> None:
        """Method for critical statement.

        Args:
            msg: Message to be printed.
            add_prefix: Boolean flag whether to include the prefix to the log message.
        """
        if add_prefix:
            print(f"{self._colors.RED}{self._format.BOLD}CRITICAL:"
                  f"{'':<3}{self._prefix()}{msg}{self._colors.END}")
        else:
            print(f"{self._colors.RED}{self._format.BOLD}CRITICAL:"
                  f"{'':<3}{msg}{self._colors.END}")
